...
class KnitMeshContoursOnMesh(component):
    ...
    def RunScript(self, Run, Mesh, KnitMeshConstraints, SamplingDensitySE, SamplingDensityLR, SamplingMode, MaxIterations, Tolerance, Threshold, VizContours):
        ...
        if not Run or MaxIterations == 0 or not Mesh or not KnitMeshConstraints:
            self.Message = "Deactivated"
            return NullTree
        
        # UNPACK CONSTRAINTS ---------------------------------------------------
        
        StartCourse = list(KnitMeshConstraints.Branch(0))
        EndCourse = list(KnitMeshConstraints.Branch(1))
        LeftBoundary = list(KnitMeshConstraints.Branch(2))
        RightBoundary = list(KnitMeshConstraints.Branch(3))
